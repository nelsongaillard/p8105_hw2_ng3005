...
In the tidy dataset `zip_zori_df`, there are a total of 17284
observations. There are 149 unique zip codes and 42 unique
neighborhoods.

There are 171 zip codes that appear in `zip_df` that do not appear in
`zori_df`. A few examples of zip codes missing from `zori_df` are 10464,
10474, 10475, 10499, 10550. These zip codes may be missing from
`zori_df` because this dataset includes data from Zillow with the zip
codes of neighborhoods with containing rental properties. It is entirely
possible that the zip codes included in `zip_df` but not `zori_df` do
not contain rental properties (i.e. perhaps these zip codes only consist
of properties you can purchase). Similarly, these zip codes may
encapsulate neighborhoods in which no people live.
...
From the newly generated table, the neighborhood with the greatest rent
drop between January 2020 and January 2021 was Lower Manhattan with a
rent drop of over \$912.
