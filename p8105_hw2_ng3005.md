...
``` r
zip_zori_df =
  left_join(zip_df, zori_df, by = c("zip_code", "county")) |>
  janitor::clean_names() |>
  relocate(county, zip_code, neighborhood, zori)
```

In the tidy dataset `zip_zori_df`, there are a total of 17342
observations. There are 320 unique zip codes and 43
